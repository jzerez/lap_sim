classdef Track
    ...
    methods
        function self = Track(track_img,apex_points)
            ...
            %   apex_points: a .csv file containing the positions of the
            %       apexes found manually
            % 
            % Returns: 
            %   self.num_points: 2D array that contains the list of points
            %   	that define the track (each column is a new point, row
            %   	1 is the x-coordinate, row 2 is the y-coordinate)
            %   self.curvatuer: the curvature at each point along the track
        ...
        function plot(self)
            ...
            zz = zeros([2, length(self.num_points)]);
            figure
            hold on
            h = surf([self.num_points(1,:); self.num_points(1,:)], [self.num_points(2,:); self.num_points(2,:)],zz,[self.curvature; self.curvature],'EdgeColor','interp');
            set(h, 'LineWidth', 2.5)
            plot(self.num_points(1,self.apex),self.num_points(2,self.apex),'bd')
            colormap jet
            colorbar
            hold off
        end
